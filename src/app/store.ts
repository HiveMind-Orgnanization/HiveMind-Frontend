 ...
 *  so they can't leak across wallets. Per-wallet variants replace them. Workspace
 *  snapshots use a different strategy — see loadWorkspaceSnapshot's first-claim
 *  migration, which preserves chat history for the wallet that first opens the
 *  mission while preventing leak to subsequent wallets. */
